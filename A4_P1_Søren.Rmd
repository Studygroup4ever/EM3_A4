...
p_load(ggplot2, tidyverse, dplyr, groupdata2, strex, zoo, readr)
```
...
df <- df %>% 
  mutate(prev_HR1 = lag(HR1_c_scaled),
         prev_HR2 = lag(HR2_c_scaled),
         prev_Resp1 = lag(Resp1_c_scaled),
         prev_Resp2 = lag(Resp2_c_scaled))

# Generate a column for each: change in HR1, HR2, Resp1, Resp2
df <- df %>% 
  mutate(change_HR1 = HR1_c_scaled-prev_HR1,
         change_HR2 = HR2_c_scaled-prev_HR2,
         change_Resp1 = Resp1_c_scaled-prev_Resp1,
         change_Resp2 = Resp2_c_scaled-prev_Resp2)
...
# Save the data
#write_csv(pitch_data, "/Users/cleme/Desktop/3. Semester CogSci/Exp Meth 3/Portfolios/Portfolio 2/A3-P2/pitch_data.csv")
```

```{r}
#df <- read_csv('ECG.data.csv')
```
...
long$HR_other <-
  tidyr::pivot_longer(df, c(HR2_c_scaled, HR1_c_scaled))[['value']]

long$HR_self_lag <-
  tidyr::pivot_longer(df, c(prev_HR1, prev_HR2))[['value']]

long$HR_other_lag <-
  tidyr::pivot_longer(df, c(prev_HR2, prev_HR1))[['value']]

long$Resp_self <-
  tidyr::pivot_longer(df, c(Resp1_c_scaled, Resp2_c_scaled))[['value']]

long$Resp_other <-
  tidyr::pivot_longer(df, c(Resp2_c_scaled, Resp1_c_scaled))[['value']]

long$Resp_self_lag <-
  tidyr::pivot_longer(df, c(prev_Resp1, prev_Resp2))[['value']]

long$Resp_other_lag <-
  tidyr::pivot_longer(df, c(prev_Resp2, prev_Resp1))[['value']]
