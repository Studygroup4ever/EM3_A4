...
removeOuts <- function(ts, threshold){
  ...
  ts[higher_threshold_condition] <- mean(ts, na.rm= T) + (threshold*sd(ts, na.rm = T))  # we replace 
...
# Will also make a function without using the removeOuts, so I can plot before/after outlier removal: 
```{r}
# Function for data preprocessing - same as above, but without removal of outliers 
...
# Clean - Identifying all files to be read
```{r}
# CLEAN: 
...
# Shuffled pairs:
When creating shuffled controls, we break the temporal dependencies. This ensures the "coordination" observed is due to the actual values in the series and not their sequence.
...
## Surrogate pairs:
When creating surrogate pairs, we isolate the effects of the task itself. 
...
# Let's do some modelling 3
Third, we will model on the surrogate pairs. 
```{r}
...
PUT MODEL CODE HER, og er der andet der mangler?
...
for (i in 1:nrow(SurrogateList)) {
  x <- subset(df, Group == SurrogateList$a[i])
  y <- subset(df, Group == SurrogateList$b[i])
  group <- 800 + i
