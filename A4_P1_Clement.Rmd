...
#How many group do we have before removing NAs?
ECG_data1 %>%
  group_by(Group) %>% 
  count(Condition, sort = T) %>% 
  View()#We have 97 groups

#TRYING TO REMOVE ALL NAs
ECG_data1 <- na.omit(ECG_data1)

#How many groups were completely removed?

ECG_data1 %>%
  group_by(Group) %>% 
  count(Condition, sort = T) #We now only have 56 groups. 41 groups were removed completely 
...
ECG_data2 <- ECG_data1
#22 change
ECG_data2 <- gather(ECG_data2, "change_HR", 23:24)

# Set the most interesting contrast e.g. by defining synchronous or conversation as the baseline

# Model change as a function of own and other previous state 
model <- lmerTest::lmer(change_HR1~ prev_HR1 + prev_HR2 + Condition + (1|Group), ECG_data2)
summary(model)
