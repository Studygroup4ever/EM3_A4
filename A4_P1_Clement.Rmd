...
```{r}
# Load the libraries
pacman::p_load(tidyverse, groupdata2, strex, zoo)
...
ECG_data1 %>%
  filter(Group == ) %>% 
  ggplot() + 
  geom_path(aes(time, HR1_c_scaled, color = "P1")) + 
  geom_path(aes(time, HR2_c_scaled, color = "P2")) + 
  labs(x = "time", y = "HR") + theme_classic()
