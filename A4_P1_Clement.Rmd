...
long$HR_self_lag <- tidyr::pivot_longer(ECG_data2, c(prev_HR1:prev_HR2))[['value']]

long$HR_other_lag <- tidyr::pivot_longer(ECG_data2, c(prev_HR2:prev_HR1))[['value']]

#Making a participant column
long$alternate <- as.numeric(rep(c(1,2), 17895))
...
model <- lmerTest::lmer(HR_self ~ HR_self_lag + HR_other_lag + Condition + (1+Participant|Group), data = subset_study1)
