...
# Make long
# HR
long <- tidyr::pivot_longer(ECG,c(HR1_c_scaled,HR2_c_scaled), values_to = "HRself")
...
HR_model1 <- lm(change_HR1 ~ 0 + (HRself + HR_other):Condition, subset(long, Study == "3" & Participant == 3022))
summary(HR_model1)
...
p <- long %>% 
  ...
  ggplot() + 
  geom_line(aes(time, HRself, color = "HRself")) + # line for self
...
# Add random effect
HR_model0 <- lmerTest::lmer(change_HR1 ~ 0 + (HRself + HR_other):Condition + (0+Condition|Participant) + (0+Condition|Group), subset(long, Study == "3"), REML = F, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
