...
```{r}
# Load the libraries
pacman::p_load(dplyr,tidyverse,readr,ggplot2,groupdata2,cowplot, stringr,purrr,tidyr,reshape2,crqa)
...
```{r}
# Make long
longHR <- tidyr::pivot_longer(ECG,c(HR1_c_scaled,HR2_c_scaled), values_to = "HRself")
longHR$other <- tidyr::pivot_longer(ECG,c(HR2_c_scaled,HR1_c_scaled), values_to = "HRother")[['HRother']] #two square brackets

longRESP <- tidyr::pivot_longer(ECG,c(Resp1_c_scaled,Resp2_c_scaled), values_to = "RESPself")
longRESP$other <- tidyr::pivot_longer(ECG,c(Resp2_c_scaled,Resp1_c_scaled), values_to = "RESPother" )[['RESPother']] #only concerning 'value'
...
ECG <- ECG %>% 
  mutate(
    HR1_c_s_lag = lag(HR1_c_scaled),
    HR2_c_s_lag = lag(HR2_c_scaled),
    Resp1_c_s_lag = lag(Resp1_c_scaled),
    Resp2_c_s_lag = lag(Resp2_c_scaled))

ECG <- ECG %>% 
  mutate(
    change_HR1 = HR1_c_scaled - HR1_c_s_lag,
    change_HR2 = HR2_c_scaled - HR2_c_s_lag,
    change_Resp1 = Resp1_c_scaled - Resp1_c_s_lag,
    change_Resp2 = Resp2_c_scaled - Resp2_c_s_lag)

# Make the data long, so we can analyze both participants at the same time. Tip: you can use the function gather () 
## N.B. This is a bit tricky and you might have to do it in several steps. 
ECG_long <- ECG %>% 
  select(Study, Trial, Group, Condition, HR1_c_scaled, HR2_c_s_lag, time) %>% 
  group_by(Study, Trial, Group) %>% 
  gather(key = "measure", value = "HR", HR1_c_scaled,HR2_c_s_lag)

# Set the most interesting contrast e.g. by defining synchronous or conversation as the baseline
ECG$Condition <- as.factor(ECG$Condition)
...
### TRICKY! Creating controls: surrogate pair controls
 ...
 So a surrogate pair is person A from pair 1, and person B from pair 2. ?? Not same task? 
